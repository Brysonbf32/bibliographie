...
<html dir="ltr" lang="en">
  ...
  <body>
    ...
    >
            ...
            <li class="sidebar-item">
              ...
              <li class="sidebar-item">
                <a
                  ...
                  href="Conge.php"
                  aria-expanded="false"
                  ><i class="mdi mdi-account-off \f111"></i
        ...
        <div class="page-breadcrumb">
          <div class="row">
            <div class="col-12 d-flex no-block align-items-center">
              <h4 class="page-title">Tables <?=$usernamerh?> <?=$idsessio?></h4>
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                   <p> <button type="button" class="btn btn-secondary btn-sm add_itemsrh" data-bs-toggle="modal" data-bs-target="#adddemande" data-bs-whatever="@mdo">Demande</button></p>
                   </div>
                <div class="container">
                  <div class="row">
                    <div class="col-12 col-lg-6 pb-2">
                      <h2 class="nom_format">Info Etudiants</h2>
                        <div class="table-responsive">
                          <table class="table">
                            <thead>
                              <tr>
                                <th class="th_tableenca" scope="col">ID COURS</th>
                                <th class="th_tableenca" scope="col">TITRE</th>
                                <th class="th_tableenca" scope="col">REF</th>
                              </tr>
                            </thead>
                                  <?php
                                    $getdata=$my_bd->query("SELECT * FROM tbl_affecta_cou_encdr Where user_id='$idsessio'");
                                    if($getdata->rowCount() >0){
                                      while($check=$getdata->fetch()){
                                        $id_cours=$check['id_cou'];
                                        $ref_aff=$check['ref_affect'];
                                        if(isset($id_cours)){
                                          $getdatadepart=$my_bd->query("SELECT * FROM tbl_cours WHERE id_cou='$id_cours'");
                                          $getdatadepart->rowCount();
                                          $checkdepa=$getdatadepart->fetch();
                                          if($checkdepa >0){
                                            $tt_cours=$checkdepa['titre_cou']; 
                                          }
                                        }
                                  ?>
                            <tbody>
                              <tr>
                                <td class="td_table"><?=$id_cours?></td>
                                <td class="td_table"><?=$tt_cours?></td>
                                <td class="td_table"><?=$ref_aff?></td>
                            </tbody>
                            <?php
                                  }
                                }
                    ...
                    <div class="col-12 col-lg-6">
                      <h2 class="nom_format">Info Cours</h2>
                        <div class="table-responsive">
                          <table class="table">
                            <thead>
                              <tr>
                                <!-- <th class="th_tableenca" scope="col">Reference</th> -->
                                <th class="th_tableenvca" scope="col">Cours</th>
                                <th class="th_tableevnca" scope="col">Heure</th>
                                <th class="th_tableevnca" scope="col">Description cours</th>
                              </tr>
                            </thead>
                            <?php
                                $getdata=$my_bd->query("SELECT * FROM tbl_affecta_cou_encdr");
                                if($getdata->rowCount() >0){
                                  while($checkdata=$getdata->fetch()){
                                    $idenca=$checkdata['id_enca'];
                                    $idcou=$checkdata['id_cou'];
                                    if(isset($idcou)){
                                      $fetchdata=$my_bd->query("SELECT * FROM tbl_cours WHERE id_cou='$idcou'");
                                      $fetchdata->rowCount();
                                      $checkdat=$fetchdata->fetch();
                                      if($checkdat >0){
                                        $titre=$checkdat['titre_cou'];
                                        $desccours=$checkdat['desc_cou'];
                                        $heure=$checkdat['duree_cour'];
                                    }
                                    }
                                  ?>
                            <tbody>
                              <tr>
                                <td class="td_table"><?=$titre?></td>
                                <td class="td_table"><?=$heure?></td>
                                <td class="td_table"><details><?=$desccours?></details></td>
                            </tbody>
                            <?php
                             }
                              }
      ...
      <!-- Add user -->
  <div class="modal fade" id="adddemande" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              ...
              <form method="POST">
                ...
                <div class="row">
                  <div class="col-12 col-lg-6">
                    ...
                    <input type="text" class="modal_input" name="input_niveo">
                  </div>
                    <div class="col-12 col-lg-6">
                    <label for="recipient-name" class="modal_labeltext">Matricule:</label>
                    <input type="text" class="modal_input" name="input_matri">
                  </div>
                </div>
                <div class="row">
                  <div class="col-12 col-lg-6">
                    <label for="recipient-name" class="modal_labeltext">Experience:</label>
                    <input type="text" class="modal_input" name="input_expe">
                  </div>
                    <div class="col-12 col-lg-6">
                    <label for="recipient-name" class="modal_labeltext">Date Naissance:</label>
                    <input type="date" class="modal_input" name="input_datenai">
                  </div>
                </div>
                <div class="row">
                <div class="col-12 col-lg-6">
                    <label for="recipient-name" class="modal_labeltext">Status</label>
                    <select name="input_staformat" class="modal_input">
                      <option value="Celibataire">Celibataire</option>
                      <option value="Mairie">Mairie</option>
                    </select>
                  </div>
                  <div class="col-12 col-lg-6">
                    <label for="recipient-name" class="modal_labeltext">Nationnalite:</label>
                    <input type="text" class="modal_input" name="input_natio">
                  </div>
                </div>
                <div class="mb-3">
