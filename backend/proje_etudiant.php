...
if(isset($_POST['ajouter'])){
    ...
    if(isset($datepu) and !empty($etudi) and !empty($proje)){
        ...
        if($checkdata >0){
            ...
            $path='../../photos/projets/'.$file_name;
            $extension_allowed=array('.zip','.pdf');
            if(in_array($file_extension,$extension_allowed)){
            if(move_uploaded_file($file_tmp_name, $path)){
            $insert=$my_bd->prepare("INSERT INTO tbl_stagiaire_projet(id_etu,id_projet,date_pub,travaile_pro)VALUE(?,?,?,?)");
            $insert->execute(array($etudi,$proje,$datepu,$file_name));
            }
        }
    }
}
