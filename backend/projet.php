    ...
    if(isset($titre) and !empty($domaine) and !empty($datedb) and !empty($datefn) and !empty($descri)){
        ...
        if($checkdata >0){
            ...
            if($datefn <= $datedb){
                ...
                $insert_sta=$my_bd->prepare("INSERT INTO tbl_projets(id_domaine,nom_projet,datedb_pro,datefn_pro,image_pro,Description_pro)VALUE(?,?,?,?,?,?)");
                $insert_sta->execute(array($domaine,$titre,$datedb,$datefn,$path,$descri));
            }
        }
    }
}
